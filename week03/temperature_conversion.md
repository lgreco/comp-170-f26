...
files — `age.py` is your model. Both programs below should follow that
same shape:
...
`our_first_function.py` to validate the input first — just wrap the
result of `input()` directly in `float()`.

These are straight-line programs, same style as `age.py`. And same
rules as `age.py`: no naked numbers or strings in the computation. Any
...
`ENTRY_PROMPT` are declared before they're used in `age.py`.
...
`age.py` uses `check()` from `our_first_function.py` to reject input
...
Submit both `.py` files. Run each one at least twice with different
